  ...
  return (
      <div className="flex flex-col p-2">
        <div>
          {/* Map Container */}
          <div>
            {/* Map now triggers updateGroupByIso on click */}
            <Map countries={countriesData} onCountryClick={updateGroupByIso} />
          </div>

          {/* Search Input Container */}
          <div className="my-4">
            <label className="block text-sm/6 font-medium text-gray-900 select-none">
              Search Countries
            </label>
            <div className="mt-2">
              <input
          ...
          <div className="flex flex-row flex-wrap gap-2">
            {filteredCountries.map((country, index) => (
                <SearchResult
                    ...
                    country={country}
                    index={index}
                    onGroupClick={() => updateGroup(index)} // Keep updating by index
                />
            ))}
          </div>
        </div>
